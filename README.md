...
- 🎯 **In-app onboarding** — First-run tutorial to get you started quickly
- 🖥️ **Web config UI** — Visual configuration with drag-and-drop image upload
- 📤 **Import/Export** — Share your configuration with others
- 🧪 **Test mode** — Preview the popup in your browser without building
...
Start the visual configuration server:

```bash
cd config-ui && python3 server.py
```

Then open http://localhost:1423 in your browser. You can:
- Configure all settings with visual previews
- Drag and drop images with preview
- Import/export configurations
- Save changes directly to config.js
...
When you first launch the app, you'll see an onboarding tutorial:
1. **Welcome** — Learn how the app works
2. **Choose Images** — Select your motivation photos
3. **Set Timer** — Configure how often the popup appears
4. **Start** — Begin your focus journey

You can reset the onboarding from the Config UI if you want to go through it again.
...
|------------------------------------|---------------------------|
| Open config UI (web)               | `cd config-ui && python3 server.py` |
| Personalize the app (terminal)     | `./personalize.sh`        |
| Test in browser (no build)         | `./test-mode.sh`          |
...
| Uninstall the app                  | `./uninstall.sh`          |
...
```
resort_to_grow/
├── personalize.sh              ← Run this to set up your app (terminal wizard)
├── setup-autostart.sh          ← Builds and installs the app
├── test-mode.sh                ← Preview popup in browser (no build)
├── uninstall.sh                ← Remove app completely
├── config-ui/                  ← Web-based configuration interface
│   ├── index.html              ← Configuration UI
│   ├── styles.css              ← Dark theme styles
│   ├── app.js                  ← Frontend logic
│   └── server.py               ← Config server (port 1423)
├── app/
│   └── src/
│       ├── config.js           ← Your settings (auto-generated)
...
│   └── server.py               ← Dashboard server (port 1422)
└── README.md
```
